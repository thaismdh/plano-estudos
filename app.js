...
function autoRescheduleOverdue(saved){const today=isoDate(new Date()),overdue=saved.sessions.filter(s=>!s.done&&s.date<today);if(!overdue.length)return 0;const platformOf=s=>saved.courses.find(c=>c.id===s.courseId)?.platform||'other',loads=new Map(),fiapDates=new Set();saved.sessions.filter(s=>!s.done&&s.date>=today).forEach(s=>{loads.set(s.date,(loads.get(s.date)||0)+s.minutes);if(platformOf(s)==='fiap')fiapDates.add(s.date)});overdue.sort((a,b)=>a.date.localeCompare(b.date)||(platformOf(a)==='fiap'?-1:1));let moved=0;overdue.forEach(s=>{const platform=platformOf(s),limit=Math.max(platform==='fiap'?180:90,s.minutes);let candidate=localDate(nextWeekday(today));for(let tries=0;tries<365;tries++){if([0,6].includes(candidate.getDay())){candidate.setDate(candidate.getDate()+1);continue}const date=isoDate(candidate),load=loads.get(date)||0,allowed=load+s.minutes<=limit&&(platform==='fiap'||!fiapDates.has(date));if(allowed){s.originalDate=s.originalDate||s.date;s.date=date;s.rescheduled=true;loads.set(date,load+s.minutes);if(platform==='fiap')fiapDates.add(date);moved++;break}candidate.setDate(candidate.getDate()+1)}});if(moved)saved.lastAutoReschedule={date:today,count:moved};return moved}
...
function renderWeek(){const monday=addDays(mondayOf(new Date()),weekOffset*7),days=Array.from({length:5},(_,i)=>addDays(monday,i)),start=isoDate(days[0]),end=isoDate(days[4]),all=state.sessions.filter(s=>s.date>=start&&s.date<=end).sort(byPriority),pending=all.filter(s=>!s.done),total=pending.reduce((sum,s)=>sum+s.minutes,0),fiap=pending.filter(s=>course(s.courseId).platform==='fiap').reduce((sum,s)=>sum+s.minutes,0),etek=pending.filter(s=>course(s.courseId).platform==='etek').reduce((sum,s)=>sum+s.minutes,0),today=isoDate(new Date());$('#weekRange').textContent=`${days[0].toLocaleDateString('pt-BR',{day:'2-digit',month:'long'})} a ${days[4].toLocaleDateString('pt-BR',{day:'2-digit',month:'long',year:'numeric'})}`;$('#weekSummary').innerHTML=`<article><span>Carga pendente</span><strong>${fmtMinutes(total)}</strong></article><article class="fiap-summary"><span>FIAP</span><strong>${fmtMinutes(fiap)}</strong></article><article class="etek-summary"><span>ETEK</span><strong>${fmtMinutes(etek)}</strong></article><article><span>Dias com estudo</span><strong>${new Set(pending.map(s=>s.date)).size} de 5</strong></article>`;$('#weekGrid').innerHTML=days.map(day=>{const date=isoDate(day),sessions=all.filter(s=>s.date===date),minutes=sessions.filter(s=>!s.done).reduce((sum,s)=>sum+s.minutes,0),load=workload(minutes),dayName=day.toLocaleDateString('pt-BR',{weekday:'long'}),platformLabel=c=>c.platform==='fiap'?'FIAP':c.platform==='etek'?'ETEK':'OUTRA';return `<article class="week-day ${date===today?'today-day':''}"><header><div><span>${dayName}</span><strong>${day.getDate()}</strong></div><span class="load-tag load-${load.className}">${load.label}</span></header><div class="week-day-total">${minutes?`${fmtMinutes(minutes)} pendentes`:'Sem pendências'}</div><div class="week-sessions">${sessions.length?sessions.map(s=>{const c=course(s.courseId);return `<div class="week-session ${s.done?'done':''}"><div><span class="platform-badge platform-${c.platform||'other'}">${platformLabel(c)}</span>${s.rescheduled?'<span class="tiny-replan">REPLANEJADO</span>':''}</div><strong>${escapeHtml(s.title)}</strong><small>${escapeHtml(c.name)} • ${fmtMinutes(s.minutes)}</small></div>`}).join(''):'<div class="week-free">Dia livre para respirar 🌿</div>'}</div></article>`}).join('')}
...
$('#weekdayBtn').onclick=()=>{const weekends=moveWeekends(state.sessions),overdue=autoRescheduleOverdue(state),moved=weekends+overdue;save();toast(moved?`${moved} ${moved===1?'sessão reorganizada':'sessões reorganizadas'}`:'Seu cronograma já está em dia')};
...
$('#resetBtn').onclick=()=>{if(confirm('Restaurar o plano inicial e apagar alterações?')){state=structuredClone(initialState);save();toast('Plano restaurado')}};
