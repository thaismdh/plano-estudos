...
const initialDeadlines=[{id:'fiap-tech-challenge-2026-09',title:'Entrega do Tech Challenge',date:'2026-09-15',platform:'fiap',note:'Conferir solução, documentação e apresentação',done:false}];
...
const rows=[
['2026-07-20','fiap','Big Data','Aula 1 — Definição e os 5 Vs do Big Data',62,3682],['2026-07-22','fiap','Big Data','Aula 2 — Hadoop e Spark',73,4337],['2026-07-24','challenge','Revisão','Revisar Big Data e praticar estruturas',120],['2026-07-27','fiap','Big Data','Aula 3 — Armazenamento de dados na nuvem',140,8400],['2026-07-29','fiap','Big Data','Aula 4 — Manipulação de dados com Spark',74,4397],['2026-07-31','challenge','Planejamento','Registrar ideias e dúvidas do projeto',120],
['2026-08-03','fiap','Bancos de Dados','Aula 1 — Bancos relacionais',56,3322],['2026-08-05','fiap','Bancos de Dados','Aula 2 — Modelo Entidade-Relacionamento',23,1355],['2026-08-07','challenge','Pesquisa','Relacionar o conteúdo ao desafio',120],['2026-08-10','fiap','Bancos de Dados','Aula 3 — DW, Data Lake, Lakehouse, Mesh e Fabric',78,4676],['2026-08-12','fiap','Bancos de Dados','Aula 4 — SQL, filtros, agrupamentos e joins',85,5056],['2026-08-14','challenge','Escopo','Definir tema, dados e entregáveis',120],['2026-08-17','fiap','Bancos de Dados','Aula 5 — Bancos NoSQL',87,5161],['2026-08-19','fiap','Storage','Aula 1 — Data Warehouse',111,6619],['2026-08-21','challenge','Arquitetura','Desenhar a solução e dividir tarefas',120],
['2026-08-24','fiap','Storage','Aula 2 — Data Lake',93,5537],['2026-08-26','fiap','Storage','Aula 3 — Data Lakehouse',22,1314],['2026-08-28','challenge','Implementação','Começar scripts e documentação',120],['2026-08-31','fiap','Revisão','Revisar Big Data e bancos de dados',90],['2026-09-02','fiap','Revisão','Revisar storage, Spark e Databricks',90],['2026-09-04','challenge','Implementação','Desenvolver e testar a solução',120],['2026-09-07','fiap','Revisão final','Consolidar conceitos e pendências',90],['2026-09-09','challenge','Finalização','Fechar documentação e apresentação',90],['2026-09-11','challenge','Revisão final','Revisar e deixar pronto para envio',120],['2026-09-14','challenge','Entrega','Conferência final e envio — prazo oficial 15/09',30]
];
const baseSessions=rows.map((r,i)=>({id:`s${i+1}`,date:r[0],courseId:r[1],module:r[2],title:r[3],minutes:r[4],...(r[5]?{durationSeconds:r[5],durationSource:'fiap-live-2026-07-20'}:{}),done:false,completedAt:null}));
const initialState={courses:initialCourses,sessions:[...pythonPlanSessions,...baseSessions],deadlines:initialDeadlines,theme:'light'};
let state=loadState();let weekOffset=0;const autoReplanCount=autoRescheduleOverdue(state);if(autoReplanCount)localStorage.setItem(STORAGE_KEY,JSON.stringify(state));let installPrompt=null;
...
function scheduleOneHourDays(saved,{initial=false}={}){
  ...
  let sequence=0;
  const all=pending.map(s=>{const original=s.plannedReleaseDate||s.originalDate||s.date,release=initial?(original<today?today:original):(s.date<today?today:s.date);return{session:s,release,sequence:sequence++,oldDate:s.date}});
...
function sessionSeconds(s){return s.durationSeconds||s.minutes*60}
function fmtSeconds(total){const h=Math.floor(total/3600),min=Math.floor(total%3600/60),sec=total%60;return `${h?`${h}h `:''}${min?`${String(min).padStart(h?2:1,'0')}min `:''}${sec?`${String(sec).padStart(h||min?2:1,'0')}s`:''}`.trim()||'0min'}
function sessionDuration(s){return s.durationSeconds?fmtSeconds(s.durationSeconds):fmtMinutes(s.minutes)}
function localDate(s){return new Date(`${s}T12:00:00`)}
function isoDate(d){return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`}
function nextWeekday(date){const d=localDate(date);while([0,6].includes(d.getDay()))d.setDate(d.getDate()+1);return isoDate(d)}
function moveWeekends(sessions){let moved=0;sessions.filter(s=>!s.done).forEach(s=>{const safe=nextWeekday(s.date);if(safe!==s.date){s.date=safe;moved++}});return moved}
function addDays(date,days){const d=new Date(date);d.setDate(d.getDate()+days);return d}
function mondayOf(date){const d=new Date(date),distance=(d.getDay()+6)%7;d.setDate(d.getDate()-distance);d.setHours(12,0,0,0);return d}
function autoRescheduleOverdue(saved){const today=isoDate(new Date()),overdue=saved.sessions.filter(s=>!s.done&&s.date<today);if(!overdue.length)return 0;const overdueIds=new Set(overdue.map(s=>s.id));scheduleOneHourDays(saved);const moved=saved.sessions.filter(s=>overdueIds.has(s.id)&&s.date>=today).length;if(moved)saved.lastAutoReschedule={date:today,count:moved};return moved}
function priority(s){return course(s.courseId).platform==='fiap'?0:1}
...
function sessionCard(s){const c=course(s.courseId),d=dateParts(s.date),fiap=c.platform==='fiap',platform=c.platform==='etek'?'ETEK':fiap?'FIAP':'OUTRA',lessons=sessionLessonItems(s);return `<article class="session-card ${s.done?'done':''} ${fiap?'fiap-priority':''}" style="--course-color:${c.color}"><div class="date-box"><strong>${d.day}</strong><span>${d.month}</span></div><div><h3>${escapeHtml(s.title)} ${fiap?'<span class="priority-tag">PRIORIDADE</span>':''}${s.rescheduled?'<span class="rescheduled-tag">REPLANEJADO</span>':''}</h3><div class="meta"><span class="platform-badge platform-${c.platform||'other'}">${platform}</span><span class="pill">${escapeHtml(c.name)}</span><span>${escapeHtml(s.module)}</span><span>${lessons.length?`${lessons.length} ${lessons.length===1?'conteúdo':'conteúdos'} • `:''}${sessionDuration(s)}</span>${s.durationSource?'<span class="pill">Duração real FIAP</span>':''}${s.actualMinutes?`<span>Realizado: ${fmtMinutes(s.actualMinutes)}</span>`:''}</div>${lessonList(s)}</div><input class="check" type="checkbox" aria-label="Marcar ${escapeHtml(s.title)} como concluído" data-session-id="${s.id}" ${s.done?'checked':''}></article>`}
function escapeHtml(v){const d=document.createElement('div');d.textContent=v;return d.innerHTML}
function render(){document.documentElement.classList.toggle('dark',state.theme==='dark');$('#themeBtn').textContent=state.theme==='dark'?'☀':'☾';const total=state.sessions.reduce((a,s)=>a+sessionSeconds(s),0),done=state.sessions.filter(s=>s.done).reduce((a,s)=>a+sessionSeconds(s),0),pct=total?Math.round(done/total*100):0;$('#totalHours').textContent=fmtSeconds(total);$('#doneHours').textContent=fmtSeconds(done);$('#remainingCount').textContent=state.sessions.filter(s=>!s.done).length;$('#progressPercent').textContent=`${pct}%`;$('#progressRing').style.strokeDashoffset=308-(308*pct/100);$('#streak').textContent=`${calcStreak()} dias úteis`;const now=new Date(),today=isoDate(now),report=state.lastAutoReschedule,showReplan=report?.date===today&&report.count>0;$('#todayLabel').textContent=now.toLocaleDateString('pt-BR',{weekday:'long',day:'2-digit',month:'long'});$('#weekendNotice').hidden=![0,6].includes(now.getDay());$('#replanNotice').hidden=!showReplan;if(showReplan)$('#replanText').textContent=`${report.count} ${report.count===1?'sessão atrasada foi movida':'sessões atrasadas foram movidas'} para os próximos dias úteis, mantendo a FIAP em primeiro lugar.`;const next=state.sessions.filter(s=>!s.done).sort(byPriority).slice(0,5);$('#heroTitle').textContent=next[0]?`Próximo: ${next[0].title}`:'Plano concluído!';$('#heroSubtitle').textContent=next[0]?`${dateParts(next[0].date).full} • ${sessionDuration(next[0])}`:'Você concluiu todas as sessões.';$('#nextSessions').innerHTML=next.length?next.map(sessionCard).join(''):'<div class="empty">Nenhuma sessão pendente. Que conquista!</div>';renderTodayOverview(now);renderDeadlines();renderWeek();renderPlan();renderCourses();fillCourseOptions();fillStudyOptions();bindChecks();bindDeadlineChecks()}
function renderTodayOverview(now){const today=isoDate(now),sessions=state.sessions.filter(s=>!s.done&&s.date===today).sort(byPriority),seconds=sessions.reduce((a,s)=>a+sessionSeconds(s),0),activities=sessions.reduce((n,s)=>n+(sessionLessonItems(s).length||1),0),weekend=[0,6].includes(now.getDay());$('#todayPlan').textContent=weekend?'Descanso':activities?`${activities} ${activities===1?'conteúdo':'conteúdos'}`:'Agenda livre';$('#todayDetail').textContent=weekend?'Sábado e domingo são seus':sessions.length?sessions.map(s=>course(s.courseId).name).filter((v,i,a)=>a.indexOf(v)===i).join(' + '):'Nenhum estudo pendente para hoje';$('#todayMinutes').textContent=fmtSeconds(seconds);if(seconds){const finish=new Date(now.getTime()+seconds*1000);$('#finishTime').textContent=finish.toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'});$('#finishDetail').textContent=`considerando início às ${now.toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})}`}else{$('#finishTime').textContent='—';$('#finishDetail').textContent=weekend?'Aproveite seu descanso':'Sem estudos pendentes'}const fiap=sessions.find(s=>course(s.courseId).platform==='fiap');$('#todayFocus').textContent=fiap?'FIAP':sessions.length?course(sessions[0].courseId).name:'FIAP'}
function deadlineCountdown(date){const days=Math.round((localDate(date)-localDate(isoDate(new Date())))/86400000);if(days<0)return{days,label:`Atrasado há ${Math.abs(days)} ${Math.abs(days)===1?'dia':'dias'}`,level:'urgent'};if(days===0)return{days,label:'Vence hoje',level:'urgent'};if(days===1)return{days,label:'Vence amanhã',level:'urgent'};return{days,label:`Faltam ${days} dias`,level:days<=7?'urgent':days<=30?'attention':'calm'}}
function renderDeadlines(){const list=state.deadlines.filter(d=>!d.done).sort((a,b)=>a.date.localeCompare(b.date)).slice(0,3);$('#deadlineList').innerHTML=list.length?list.map(d=>{const count=deadlineCountdown(d.date),platform=d.platform==='etek'?'ETEK':'FIAP';return `<article class="deadline-card deadline-${count.level}"><div class="deadline-date"><strong>${localDate(d.date).getDate()}</strong><span>${localDate(d.date).toLocaleDateString('pt-BR',{month:'short'}).replace('.','')}</span></div><div class="deadline-copy"><div><span class="platform-badge platform-${d.platform}">${platform}</span><span class="deadline-countdown">${count.label}</span></div><h4>${escapeHtml(d.title)}</h4>${d.note?`<p>${escapeHtml(d.note)}</p>`:''}</div><input class="deadline-check" type="checkbox" aria-label="Marcar o prazo ${escapeHtml(d.title)} como concluído" data-deadline-id="${d.id}"></article>`}).join(''):'<div class="deadline-empty">Todos os prazos foram concluídos 🎉</div>'}
function workload(minutes){if(!minutes)return{label:'Livre',className:'free'};if(minutes<=45)return{label:'Leve',className:'light'};if(minutes<=90)return{label:'Equilibrada',className:'balanced'};return{label:'Intensa',className:'intense'}}
function renderWeek(){
  const monday=addDays(mondayOf(new Date()),weekOffset*7),days=Array.from({length:5},(_,i)=>addDays(monday,i)),start=isoDate(days[0]),end=isoDate(days[4]),all=state.sessions.filter(s=>s.date>=start&&s.date<=end).sort(byPriority),pending=all.filter(s=>!s.done),total=pending.reduce((sum,s)=>sum+sessionSeconds(s),0),fiap=pending.filter(s=>course(s.courseId).platform==='fiap').reduce((sum,s)=>sum+sessionSeconds(s),0),etek=pending.filter(s=>course(s.courseId).platform==='etek').reduce((sum,s)=>sum+sessionSeconds(s),0),today=isoDate(new Date());
  $('#weekRange').textContent=`${days[0].toLocaleDateString('pt-BR',{day:'2-digit',month:'long'})} a ${days[4].toLocaleDateString('pt-BR',{day:'2-digit',month:'long',year:'numeric'})}`;
  $('#weekSummary').innerHTML=`<article><span>Carga pendente</span><strong>${fmtSeconds(total)}</strong></article><article class="fiap-summary"><span>FIAP</span><strong>${fmtSeconds(fiap)}</strong></article><article class="etek-summary"><span>ETEK</span><strong>${fmtSeconds(etek)}</strong></article><article><span>Dias com estudo</span><strong>${new Set(pending.map(s=>s.date)).size} de 5</strong></article>`;
  $('#weekGrid').innerHTML=days.map(day=>{const date=isoDate(day),sessions=all.filter(s=>s.date===date),seconds=sessions.filter(s=>!s.done).reduce((sum,s)=>sum+sessionSeconds(s),0),load=workload(Math.ceil(seconds/60)),dayName=day.toLocaleDateString('pt-BR',{weekday:'long'}),platformLabel=c=>c.platform==='fiap'?'FIAP':c.platform==='etek'?'ETEK':'OUTRA';return `<article class="week-day ${date===today?'today-day':''}"><header><div><span>${dayName}</span><strong>${day.getDate()}</strong></div><span class="load-tag load-${load.className}">${load.label}</span></header><div class="week-day-total">${seconds?`${fmtSeconds(seconds)} pendentes`:'Sem pendências'}</div><div class="week-sessions">${sessions.length?sessions.map(s=>{const c=course(s.courseId);return `<div class="week-session ${s.done?'done':''}"><div><span class="platform-badge platform-${c.platform||'other'}">${platformLabel(c)}</span>${s.rescheduled?'<span class="tiny-replan">REPLANEJADO</span>':''}</div><strong>${escapeHtml(s.title)}</strong><small>${escapeHtml(c.name)} • ${sessionDuration(s)}</small></div>`}).join(''):'<div class="week-free">Dia livre para respirar 🌿</div>'}</div></article>`}).join('')
}
function renderPlan(){const platform=$('#platformFilter').value||'all',track=$('#trackFilter').value||'all',status=$('#statusFilter').value||'all';const list=state.sessions.filter(s=>{const c=course(s.courseId);return(platform==='all'||c.platform===platform)&&(track==='all'||s.courseId===track)&&(status==='all'||(status==='done'&&s.done)||(status==='pending'&&!s.done))}).sort(byPriority);let month='';$('#allSessions').innerHTML=list.map(s=>{const m=localDate(s.date).toLocaleDateString('pt-BR',{month:'long',year:'numeric'});const head=m!==month?`<div class="timeline-month">${month=m}</div>`:'';return head+sessionCard(s)}).join('')||'<div class="empty">Nenhuma sessão encontrada para esses filtros.</div>';bindChecks()}
function courseMetrics(c){const ss=state.sessions.filter(s=>s.courseId===c.id),done=ss.filter(s=>s.done).length,lessonCount=ss.reduce((n,s)=>n+sessionLessonItems(s).length,0),doneLessons=ss.filter(s=>s.done).reduce((n,s)=>n+sessionLessonItems(s).length,0),p=lessonCount?Math.round(doneLessons/lessonCount*100):ss.length?Math.round(done/ss.length*100):0;return{ss,done,p,lessonCount,doneLessons,minutes:ss.reduce((a,s)=>a+s.minutes,0),seconds:ss.reduce((a,s)=>a+sessionSeconds(s),0)}}
function compactCourse(c){const m=courseMetrics(c);return `<article class="mini-course" style="--course-color:${c.color}"><div><span class="mini-dot"></span><h4>${escapeHtml(c.name)}</h4><p>${m.ss.length} sessões • ${fmtSeconds(m.seconds)}</p></div><strong>${m.p}%</strong><div class="mini-progress"><span style="width:${m.p}%"></span></div></article>`}
...
function fillStudyOptions(){const pending=state.sessions.filter(s=>!s.done).sort(byPriority).slice(0,20);$('#studySession').innerHTML=pending.map(s=>`<option value="${s.id}">${escapeHtml(dateParts(s.date).full)} — ${escapeHtml(s.title)}</option>`).join('')||'<option value="">Nenhuma sessão pendente</option>'}
...
function bindDeadlineChecks(){$$('.deadline-check').forEach(el=>el.onchange=()=>{const d=state.deadlines.find(x=>x.id===el.dataset.deadlineId);if(!d)return;d.done=el.checked;d.completedAt=el.checked?new Date().toISOString():null;save();toast('Prazo concluído!')})}
function businessDaysBetween(a,b){let count=0,d=new Date(b);while(d<a){d.setDate(d.getDate()+1);if(![0,6].includes(d.getDay()))count++}return count}
function calcStreak(){const days=[...new Set(state.sessions.filter(s=>s.done&&s.completedAt).map(s=>s.completedAt.slice(0,10)))].sort().reverse();if(!days.length)return 0;let streak=1;for(let i=1;i<days.length;i++){const a=localDate(days[i-1]),b=localDate(days[i]);if(businessDaysBetween(a,b)===1)streak++;else break}return streak}
...
$('#prevWeek').onclick=()=>{weekOffset--;renderWeek()};$('#nextWeek').onclick=()=>{weekOffset++;renderWeek()};$('#currentWeek').onclick=()=>{weekOffset=0;renderWeek()};
...
$('#sessionForm').onsubmit=e=>{e.preventDefault();const f=new FormData(e.target),minutes=parseDuration(f.get('duration')),date=f.get('date');if(!minutes){toast('Informe a duração como 28:23 ou 60');return}if([0,6].includes(localDate(date).getDay())){toast('Escolha um dia útil para preservar seu descanso');return}state.sessions.push({id:`s${Date.now()}`,date,courseId:f.get('course'),module:f.get('module'),title:f.get('title'),minutes,done:false,completedAt:null});scheduleOneHourDays(state);save();e.target.reset();$('#sessionModal').close();toast('Sessão adicionada respeitando o limite diário')};
$('#studyForm').onsubmit=e=>{e.preventDefault();const f=new FormData(e.target),s=state.sessions.find(x=>x.id===f.get('session'));if(!s){toast('Nenhuma sessão selecionada');return}s.done=true;s.completedAt=new Date().toISOString();s.actualMinutes=Number(f.get('actualMinutes'));s.note=String(f.get('note')||'').trim();save();e.target.reset();$('#studyModal').close();toast('Estudo de hoje registrado!')};
